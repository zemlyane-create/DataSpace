...
export default function App() {
  ...
  const [newspaperNotes, setNewspaperNotes] = useState<NewspaperNote[]>(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : INITIAL_NEWSPAPER_NOTES;
    } catch {
      return INITIAL_NEWSPAPER_NOTES;
    }
  });
    ...
    async function initSupabaseDataAndAuth() {
      try {
        ...
        if (!recordsError && recordsData && recordsData.length > 0) {
          const pendingOffline = getPendingRecords();
          if (pendingOffline.length > 0) {
            const pendingIds = new Set(pendingOffline.map(p => p.id));
            const merged = [
              ...pendingOffline,
              ...recordsData.filter(r => !pendingIds.has(r.id))
            ];
            setRecords(merged);
          } else {
